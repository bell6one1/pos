...
// ==========================================
// PENGATURAN VIP & VARIABEL GLOBAL (Diperbarui)
// ==========================================
let globalSettings = {
...
// 💡 UPDATE: Perhitungan Dinamis dengan Pajak dan Service Charge
                ...
                for (const sale of pendingSales) {
                    try {
                        ...
                        if (sale.tipe === "pelunasan_piutang") {
                            ...
                            if (sale.memberId && sale.metodePembayaran === "Kasbon") { await updateDoc(doc(db, "members", sale.memberId), { hutang: increment(sale.totalAkhir) }); } 
                            else if (sale.memberId && sale.metodePembayaran !== "Kasbon") { const addPoin = Math.floor(sale.totalAkhir / (globalSettings.kelipatanPoin || 10000)); if (addPoin > 0) await updateDoc(doc(db, "members", sale.memberId), { poin: increment(addPoin) }); }
...
window.tambahVoucherAdmin = async () => {
    ...
    try { await setDoc(doc(db, "pengaturan", "global"), { vouchers: currentVouchers }, { merge: true }); document.getElementById('new-voucher-code').value = ""; document.getElementById('new-voucher-value').value = ""; alert("Voucher berhasil ditambahkan!"); } catch(e) { alert("Gagal menyimpan voucher."); }
};
...
window.duplikatBarang = (id) => {
    ...
    document.getElementById('item-id').value = ""; document.getElementById('item-barcode').value = ""; document.getElementById('item-name').value = item.nama + " (Copy)"; document.getElementById('item-category').value = item.kategori || "Umum"; document.getElementById('item-notes').value = item.catatan || ""; document.getElementById('item-cost').value = formatInputRibuan(item.cost); document.getElementById('item-price').value = formatInputRibuan(item.harga); document.getElementById('item-stock').value = formatInputRibuan(item.stok);
...
window.editBarang = (id) => {
    const item = databaseBarang.find(x => x.id === id); if (!item) return;
    document.getElementById('item-id').value = item.id; document.getElementById('item-barcode').value = item.barcode || ""; document.getElementById('item-name').value = item.nama; document.getElementById('item-category').value = item.kategori || "Umum"; document.getElementById('item-notes').value = item.catatan || ""; document.getElementById('item-cost').value = formatInputRibuan(item.cost); document.getElementById('item-price').value = formatInputRibuan(item.harga); document.getElementById('item-stock').value = formatInputRibuan(item.stok);
    const supSelect = document.getElementById('item-supplier'); if (supSelect) supSelect.value = item.supplierId || "";
...
document.getElementById('pay-method-cash')?.addEventListener('click', () => { resetPaymentUI(); selectedPaymentMethod = 'Tunai'; const btn = document.getElementById('pay-method-cash'); if(btn) btn.className = "py-2.5 text-[11px] font-bold bg-mantine-blue text-white rounded-md transition-all shadow"; document.getElementById('cash-paid')?.classList.remove('hidden'); document.getElementById('quick-cash-zone')?.classList.remove('hidden'); hitungUangKembalian(); });
document.getElementById('pay-method-noncash')?.addEventListener('click', () => { resetPaymentUI(); selectedPaymentMethod = 'Non-Tunai'; const btn = document.getElementById('pay-method-noncash'); if(btn) btn.className = "py-2.5 text-[11px] font-bold bg-mantine-blue text-white rounded-md transition-all shadow"; document.getElementById('noncash-ref')?.classList.remove('hidden'); hitungUangKembalian(); });
document.getElementById('pay-method-kasbon')?.addEventListener('click', () => { if(!activeMember) { alert("Pilih Pelanggan terlebih dahulu untuk melakukan Kasbon!"); return; } resetPaymentUI(); selectedPaymentMethod = 'Kasbon'; const btn = document.getElementById('pay-method-kasbon'); if(btn) btn.className = "py-2.5 text-[11px] font-bold bg-amber-500 text-white rounded-md transition-all shadow"; hitungUangKembalian(); });

document.getElementById('pay-method-split')?.addEventListener('click', () => {
    if(keranjang.length === 0) return alert("Keranjang kosong!");
    resetPaymentUI(); isSplitPayment = true; selectedPaymentMethod = 'Split'; const btn = document.getElementById('pay-method-split'); if(btn) btn.className = "w-full py-2.5 mb-3 text-[11px] font-bold text-white bg-mantine-blue rounded-lg shadow-lg shadow-mantine-blue/30 transition-all";
    document.getElementById('split-total-tagihan').textContent = toRupiah(globalGrandTotal); document.getElementById('split-amount-1').value = ""; document.getElementById('split-amount-2').value = formatInputRibuan(globalGrandTotal); document.getElementById('split-modal').classList.remove('hidden');
});
...
document.getElementById('split-amount-1')?.addEventListener('input', (e) => { const val1 = Math.max(0, parseInputRibuan(e.target.value)); const sisa = Math.max(0, globalGrandTotal - val1); document.getElementById('split-amount-2').value = formatInputRibuan(sisa); });
document.getElementById('btn-save-split')?.addEventListener('click', () => {
    const v1 = Math.max(0, parseInputRibuan(document.getElementById('split-amount-1').value)); const v2 = Math.max(0, parseInputRibuan(document.getElementById('split-amount-2').value)); const totalBayarSplit = v1 + v2;
    ...
    document.getElementById('split-modal').classList.add('hidden'); document.getElementById('btn-checkout').disabled = false; document.getElementById('btn-checkout').textContent = kembalianSplit > 0 ? `BAYAR (KEMBALI ${toRupiah(kembalianSplit)})` : `BAYAR (SPLIT)`;
});
...
document.getElementById('btn-hold-bill')?.addEventListener('click', () => {
    ...
    keranjang = []; localStorage.removeItem("pos_recovery_cart"); activeMember = null; localStorage.removeItem("pos_recovery_member"); appliedVoucher = null; if(document.getElementById('voucher-code')) document.getElementById('voucher-code').value = ""; document.getElementById('btn-remove-member')?.click(); renderKeranjang(); updateHoldCountBadge(); alert("Pesanan ditangguhkan."); window.switchCartTab('list');
});
...
window.loadHeldBill = async (id) => {
    ...
    if (idx > -1) {
        ...
        for (let item of bill.items) { const dbItem = databaseBarang.find(i => i.id === item.id); if (!dbItem || (dbItem.stok || 0) <= 0) { hasChanges = true; continue; } if (item.qty > dbItem.stok) { item.qty = dbItem.stok; hasChanges = true; } if (item.harga !== dbItem.harga) { item.harga = dbItem.harga; hasChanges = true; } if (item.cost !== dbItem.cost) { item.cost = dbItem.cost || 0; hasChanges = true; } validatedItems.push(item); }
        ...
        activeMember = null;
        if (bill.activeMember && bill.activeMember.id) { if(navigator.onLine) { try { const snap = await getDoc(doc(db, "members", bill.activeMember.id)); if(snap.exists()) activeMember = { id: snap.id, ...snap.data() }; } catch(e) { activeMember = bill.activeMember; } } else { activeMember = bill.activeMember; } localStorage.setItem("pos_recovery_member", JSON.stringify(activeMember)); showActiveMemberUI(); } else { document.getElementById('btn-remove-member')?.click(); }
        heldBills.splice(idx, 1); localStorage.setItem('pos_held_bills', JSON.stringify(heldBills)); document.getElementById('hold-modal')?.classList.add('hidden'); renderKeranjang(); updateHoldCountBadge(); window.switchCartTab('list');
    }
};
...
window.startVoiceSearchGudang = () => {
    const btn = document.getElementById('btn-voice-search'), SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition; if (!SpeechRecognition) { alert("Maaf, browser Anda tidak mendukung Pencarian Suara."); return; }
...
window.startVoiceSearchKasir = () => {
    const btn = document.getElementById('btn-voice-search-kasir'), SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition; if (!SpeechRecognition) { alert("Maaf, browser Anda tidak mendukung Pencarian Suara."); return; }
    const recognition = new SpeechRecognition(); recognition.lang = 'id-ID'; recognition.interimResults = false; recognition.maxAlternatives = 1;
    recognition.onstart = function() { if(btn) { btn.classList.add('bg-red-500', 'animate-pulse'); btn.textContent = "🎙️"; } };
...
window.bukaModalBayarPiutang = (memberId) => {
    ...
    document.getElementById('piutang-member-name').textContent = piutangAktifDipilih.nama.toUpperCase(); document.getElementById('piutang-sisa-hutang').textContent = toRupiah(piutangAktifDipilih.hutang); document.getElementById('piutang-bayar-input').value = ""; document.getElementById('bayar-piutang-modal').classList.remove('hidden');
};
