...
let unsubscribeItems = null, unsubscribeSales = null, unsubscribeShifts = null, unsubscribeAudit = null;
let filterKategoriAktif = "Semua";
let kataKunciPencarian = "";
let globalSubtotal = 0, globalDiskon = 0, globalGrandTotal = 0;
    ...
    try {
        ...
        request.onsuccess = async () => {
            ...
            if(successCount > 0) {
                ...
                alert(`Koneksi Stabil! ${successCount} data penjualan offline disinkronisasi.`);
            }
    ...
    if (e.key === 'Enter' && barcodeBuffer.length > 0) {
        ...
        if (b) { 
            if ((b.stok||0) > 0) {
                ...
                if (e.target.id === 'kasir-search') { e.target.value = ""; kataKunciPencarian = ""; renderKatalogKasir(); }
...
onAuthStateChanged(auth, async (user) => {
    ...
    if (user) {
        ...
        stopRealtimeListeners(); activeShiftSession = null; currentUserId = null; 
    }
});

document.getElementById('login-form')?.addEventListener('submit', async (e) => {
    e.preventDefault(); 
    if (!navigator.onLine) return alert("Koneksi internet dibutuhkan untuk masuk ke dalam sistem!");
    ...
    try { 
        ...
        e.target.reset(); 
    } catch (e) { alert("Login Gagal! Periksa kredensial."); } 
    finally { if(btnSubmit) { btnSubmit.disabled = false; btnSubmit.textContent = "Masuk Aplikasi"; } }
});
...
document.getElementById('btn-logout')?.addEventListener('click', async () => { 
    ...
    if(confirm("Keluar dari sistem?")) {
        ...
        finally { keranjang = []; activeMember = null; localStorage.clear(); location.reload(); } 
    } 
});
...
function switchTab(id) {
    ...
    if(targetBtn) { targetBtn.classList.remove('border-transparent', 'text-dark-1'); targetBtn.classList.add('border-mantine-blue', 'text-mantine-blue'); }
    if (id === 'dashboard' && chartInstance) setTimeout(() => chartInstance.update(), 100);
}

tabsBtns.forEach(tab => { 
    tab.addEventListener('click', () => switchTab(tab.id.replace('tab-', '').replace('-btn', ''))); 
});
...
window.triggerBukaShift = () => {
    ...
    document.getElementById('shift-input-label').textContent = "Modal Fisik (Rp)";
    document.getElementById('btn-close-shift-modal')?.classList.add('hidden'); 
    document.getElementById('btn-shift-submit').textContent = "Buka";
    const form = document.getElementById('shift-form');
    if(!form) return;
    form.onsubmit = async (e) => {
        e.preventDefault(); 
        if (!navigator.onLine) return alert("Peringatan: Tidak dapat membuka shift tanpa internet.");
        ...
        finally { if(btnSubmit) { btnSubmit.disabled = false; btnSubmit.textContent = "Buka Shift"; } form.reset(); }
    };
...
window.triggerTutupShift = () => {
    ...
    form.onsubmit = async (e) => {
        e.preventDefault(); 
        if (!navigator.onLine) return alert("Peringatan: Tidak dapat menutup shift tanpa internet.");
        const btnSubmit = document.getElementById('btn-shift-submit'); 
...
function initRealtimeListeners() {
    ...
    if (currentUserRole === 'admin') {
        unsubscribeShifts = onSnapshot(query(shiftsRef, orderBy("waktuBuka", "desc")), (snapshot) => { 
            dataShiftAll = []; 
...
window.setShortcutTanggal = (type) => {
    ...
    else { document.getElementById('filter-date-end').value = ""; document.getElementById('filter-date-start').value = ""; applyFiltersAndStats(); return; }
    
    const dStart = document.getElementById('filter-date-start');
    const dEnd = document.getElementById('filter-date-end');
    if(dStart) dStart.value = startStr; 
    if(dEnd) dEnd.value = endStr; 
    applyFiltersAndStats();
};
...
async function applyFiltersAndStats() {
    ...
    let totalOmset = 0, totalTrx = dataPenjualanTerfilter.length, totalItems = 0; 
    let produkCounts = {};
    ...
    const omsetEl = document.getElementById('dash-omset'); if (omsetEl) omsetEl.textContent = toRupiah(totalOmset);
    const trxEl = document.getElementById('dash-transaksi'); if (trxEl) trxEl.textContent = totalTrx;
    const itemsEl = document.getElementById('dash-items'); if (itemsEl) itemsEl.innerHTML = `${totalItems} <span class="text-lg font-medium text-dark-2">Item</span>`;
...
document.getElementById('btn-hold-bill')?.addEventListener('click', () => {
    ...
    let holdName = prompt("Nama Penanda (cth: Meja 5 / Bpk Andi):"); 
    ...
    heldBills.push({ id: Date.now().toString(), tag: holdName, waktu: new Date().toLocaleString('id-ID'), items: keranjang, diskon: Math.round(parseFloat(document.getElementById('cart-discount')?.value || 0)) || 0, activeMember: activeMember });
    ...
    renderKeranjang(); updateHoldCountBadge(); alert("Pesanan disimpan.");
});
...
window.loadHeldBill = (id) => {
    ...
    if (idx > -1) {
        ...
        if (validatedItems.length === 0) {
            alert("Semua barang dalam pesanan ini sudah dihapus atau stok habis di Master Data!");
        ...
        if (hasChanges) alert("Beberapa barang telah disesuaikan otomatis mengikuti Master Data terbaru.");
...
document.getElementById('btn-check-member')?.addEventListener('click', async () => {
    ...
    if (!navigator.onLine) return alert("Peringatan: Fitur pengecekan member membutuhkan internet.");
    ...
    try {
        ...
        if (docSnap.exists()) { activeMember = { id: phone, ...docSnap.data() }; localStorage.setItem("pos_recovery_member", JSON.stringify(activeMember)); showActiveMemberUI(); } 
            ...
            if (confirm(`Member ${phone} belum terdaftar. Daftarkan sekarang?`)) { 
...
document.getElementById('member-form')?.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if(checkSnap.exists()) return alert("Nomor HP ini sudah terdaftar!");
...
function renderKatalogKasir() {
    ...
    if (filtered.length === 0) { katContainer.innerHTML = `<p class=\"text-xs text-dark-2 italic col-span-full text-center py-8\">Kosong.</p>`; return; }
...
window.tambahKeKeranjang = (id) => {
    ...
    if (existing) { 
        if(existing.qty >= item.stok) return alert("Melebihi stok maksimal!"); 
        existing.qty++; 
    } else { 
...
window.ubahQtyCart = (id, delta) => {
    ...
    if (delta > 0 && !itemDb) return alert("Barang ini telah dihapus oleh Admin.");
    ...
    if (keranjang[index].qty <= 0) { 
        ...
        keranjang[index].qty = itemDb.stok||0; alert(`Stok maksimal tercapai!`); 
    }
    localStorage.setItem("pos_recovery_cart", JSON.stringify(keranjang));
    playBeep(); renderKeranjang();
};
...
function renderKeranjang() {
    ...
    if(keranjang.length === 0) {
        ...
        if(btnCheckout) { btnCheckout.disabled = true; btnCheckout.className = "w-full py-3 bg-dark-5 text-dark-3 font-bold rounded-xl cursor-not-allowed text-xs uppercase"; }
...
function hitungUangKembalian() {
    ...
    const rawDisc = parseFloat(document.getElementById('cart-discount')?.value) || 0;
    ...
    if (selectedPaymentMethod === 'Tunai') {
        const rawCash = parseFloat(document.getElementById('cash-paid')?.value) || 0;
        ...
        if(btnCheckout) {
            if (cashPaidVal >= globalGrandTotal && keranjang.length > 0) { btnCheckout.disabled = false; btnCheckout.className = "w-full py-3 bg-mantine-blue text-white font-bold rounded-xl text-xs uppercase cursor-pointer"; } 
            else { btnCheckout.disabled = true; btnCheckout.className = "w-full py-3 bg-dark-5 text-dark-3 font-bold rounded-xl cursor-not-allowed text-xs uppercase"; }
        }
    } else {
        const cRet = document.getElementById('cash-return'); if(cRet) cRet.textContent = "Rp 0";
        if(btnCheckout) {
            if(keranjang.length > 0) { btnCheckout.disabled = false; btnCheckout.className = "w-full py-3 bg-mantine-blue text-white font-bold rounded-xl text-xs uppercase cursor-pointer"; } 
            else { btnCheckout.disabled = true; btnCheckout.className = "w-full py-3 bg-dark-5 text-dark-3 font-bold rounded-xl cursor-not-allowed text-xs uppercase"; }
        }
    }
}
...
document.getElementById('btn-checkout')?.addEventListener('click', async (e) => {
    ...
    if (selectedPaymentMethod === 'Tunai' && cashPaidVal < globalGrandTotal) return alert("Transaksi ditolak. Uang tidak cukup!");
    btnCheckout.disabled = true; btnCheckout.textContent = "MEMPROSES...";
    const refCode = document.getElementById('payment-ref-code')?.value.trim() || '';
    ...
    try {
        ...
        if (isOnlineSuccess) {
            ...
            logActivity("CHECKOUT_ONLINE", `Penjualan Rp ${toRupiah(globalGrandTotal)} diproses.`); 
        } else {
            trxData.waktuLokal = new Date().toISOString();
            trxData.isOfflinePending = true;
            const isSavedLocally = await saveTransactionOffline(trxData);
            if (!isSavedLocally) throw new Error("Gagal menyimpan transaksi ke database lokal.");
            keranjang = []; localStorage.removeItem("pos_recovery_cart"); localStorage.removeItem("pos_recovery_member");
            ...
            alert("⚠️ Transaksi diselamatkan di brankas lokal browser.");
        }
    ...
    finally { btnCheckout.disabled = false; btnCheckout.textContent = "Selesaikan Transaksi"; hitungUangKembalian(); }
});
...
itemForm?.addEventListener('submit', async (e) => {
    e.preventDefault(); 
    if (!navigator.onLine) return alert("Koneksi internet dibutuhkan untuk master barang.");
    ...
    try {
        ...
        const data = { barcode: barcodeInput, nama: document.getElementById('item-name')?.value.trim(), kategori: document.getElementById('item-category')?.value.trim() || 'Umum', harga: Math.round(Math.max(0, rawHrg)), stok: Math.max(0, rawStk) };
        if(id) { 
            ...
            await logActivity("GUDANG_TAMBAH", `Produk baru [${data.nama}] qty: ${data.stok}`); 
        }
        window.resetForm();
    } catch(err) {} 
    finally { if(btnSubmit) { btnSubmit.disabled = false; btnSubmit.textContent = origText; } }
...
window.hapusBarang = async (id) => { 
    ...
    const item = databaseBarang.find(x => x.id === id); if(!item) return;
    if(confirm(`Hapus ${item.nama}?`)) { await logActivity("GUDANG_HAPUS", `Menghapus produk [${item.nama}].`); await deleteDoc(doc(db, "barang", id)); } 
};
...
function renderRiwayatTable() {
    ...
    if(dataPenjualanTerfilter.length === 0) { tbody.innerHTML = `<tr><td colspan=\"5\" class=\"px-6 py-8 text-center text-xs text-dark-2 italic\">Tidak ada transaksi.</td></tr>`; return; }
