...
function updateShiftUI(isActive) { 
    ...
    if (isActive) { 
        const namaKasirAman = (auth.currentUser?.email || 'Kasir').split('@')[0].toUpperCase();
        
        // Memakai gaya container versi lama + pointer-events-auto
        w.className = "bg-green-900/20 border border-green-800/50 p-5 rounded-xl flex flex-col sm:flex-row sm:items-center justify-between gap-4 pointer-events-auto transition-all duration-300"; 
        
        // Struktur HTML diperbaiki agar valid, style persis versi lama + hover:-translate-y-0.5
        w.innerHTML = `
            <div class="text-sm text-green-400">
                <div class="font-bold flex items-center gap-2">
                    <div class="w-2.5 h-2.5 rounded-full bg-green-500 animate-pulse"></div> 
                    Sesi Aktif: ${escapeHTML(namaKasirAman)}
                </div>
                <p class="text-green-500/80 mt-1 text-xs font-medium">
                    Laci: ${toRupiah((activeShiftSession.modalAwal||0) + (activeShiftSession.totalTunai||0))} | Omset Total: ${toRupiah(activeShiftSession.totalPenjualan || 0)}
                </p>
            </div>
            <button onclick="window.triggerTutupShift()" class="px-5 py-2.5 text-xs font-bold text-gray-100 bg-dark-5 hover:bg-red-500 hover:text-white transition-all duration-300 rounded-xl border border-dark-4 hover:border-red-600 shadow cursor-pointer pointer-events-auto hover:-translate-y-0.5">
                Tutup Sesi 🔒
            </button>
        `; 
        ...
        // Memakai gaya container versi lama + relative z-10 & pointer-events-auto
        w.className = "bg-dark-8 border border-dark-4 p-5 rounded-xl flex flex-col sm:flex-row sm:items-center justify-between gap-4 pointer-events-auto relative z-10 transition-all duration-300"; 
        
        // Tombol menggunakan class persis versi lama (shadow-lg, dll) + efek scale agar lebih "menyala"
        w.innerHTML = `
            <div class="text-sm text-dark-0">
                <p class="font-bold flex items-center gap-2">🔒 Sesi Belum Dibuka</p>
                <p class="text-dark-2 mt-1 text-xs">Buka shift terlebih dahulu untuk bertransaksi.</p>
            </div>
            <button onclick="window.triggerBukaShift()" class="px-5 py-2.5 text-xs font-bold text-white bg-mantine-blue hover:bg-mantine-hover rounded-xl shadow-lg shadow-mantine-blue/20 transition-all duration-300 cursor-pointer pointer-events-auto hover:scale-105 active:scale-95">
                Mulai Shift 🔑
            </button>
        `; 
