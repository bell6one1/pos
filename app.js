...
// ==========================================
// FUNGSI UTAMA KALKULASI & UI
// ==========================================

function resetPaymentUI() {
    ['pay-method-cash', 'pay-method-noncash', 'pay-method-kasbon'].forEach(id => { 
        const b = document.getElementById(id); 
        if(b) b.className = "py-2.5 text-[11px] font-bold text-dark-1 hover:text-gray-100 rounded-md transition-all"; 
    });
    
    const btnSplit = document.getElementById('pay-method-split'); 
    if(btnSplit) btnSplit.className = "w-full py-2.5 mb-4 text-[11px] font-bold text-mantine-blue bg-mantine-blue/10 hover:bg-mantine-blue/20 border border-mantine-blue/30 rounded-lg transition-all";
    
    document.getElementById('cash-paid')?.classList.add('hidden'); 
    document.getElementById('noncash-ref')?.classList.add('hidden'); 
    document.getElementById('quick-cash-zone')?.classList.add('hidden'); 
...
function terapkanPengaturanLayar() {
    ...
    if (document.getElementById('gudang-export-container')) {
...
function renderKatalogKasir() {
    ...
    if (filtered.length === 0) { 
        katContainer.innerHTML = `<p class="text-xs text-dark-2 italic col-span-full text-center py-8">Produk tidak ditemukan.</p>`; 
        return; 
    }
    ...
    if (filtered.length > kasirItemLimit) {
        ...
        <div class="col-span-full flex justify-center py-6">
            <button onclick="window.loadMoreKasir()" class="px-6 py-2.5 bg-dark-5 hover:bg-dark-4 text-white text-xs font-bold rounded-xl shadow transition-colors border border-dark-4">Tampilkan Lebih Banyak (${filtered.length - kasirItemLimit})</button>
        </div>`;
    }
}
...
function renderGudangList() {
    ...
    let filtered = databaseBarang.filter(i => { 
        ...
        return (i.nama || '').toLowerCase().includes(keyword) || (i.barcode || '').toLowerCase().includes(keyword) || (i.kategori || '').toLowerCase().includes(keyword) || (i.catatan || '').toLowerCase().includes(keyword); 
    });
    ...
    if(filtered.length === 0) { 
        container.innerHTML = `<p class="text-[11px] text-dark-2 italic text-center py-4">Barang tidak ditemukan.</p>`; 
        return; 
    }
    ...
    container.innerHTML = toShowGudang.map(i => {
        let supName = ""; 
        if(i.supplierId) { 
            const sup = databasePemasok.find(x => x.id === i.supplierId); 
            if(sup) supName = sup.nama; 
        }
        const isLowStock = (i.stok||0) <= (globalSettings.batasStok || 5);
        ...
        <div class="bg-dark-6 p-4 rounded-xl border border-dark-4 flex justify-between items-center gap-3 shadow-sm hover:border-dark-3 transition-colors">
            <div>
                ...
                <div class="flex items-center gap-2 mt-1.5">
                    <span class="text-xs font-black text-green-400">${toRupiah(i.harga)}</span> 
                    <span class="text-[10px] text-dark-2">| Modal: ${toRupiah(i.cost||0)}</span> 
                    <span class="text-[9px] font-bold ml-1 px-1.5 py-0.5 bg-dark-5 text-dark-0 rounded border border-dark-4 ${stockClass}">Stok: ${formatInputRibuan(i.stok||0)}</span>
                </div>
            </div>
    ...
    if (filtered.length > gudangItemLimit) {
        ...
        <div class="flex justify-center py-4">
            <button onclick="window.loadMoreGudang()" class="px-6 py-2.5 bg-dark-5 hover:bg-dark-4 text-white text-xs font-bold rounded-xl shadow transition-colors border border-dark-4">Tampilkan Lebih Banyak (${filtered.length - gudangItemLimit} item lagi)</button>
        </div>`;
    }
}
...
function renderKeranjang() {
    ...
    if(keranjang.length === 0) {
        ...
        if(document.getElementById('kembalian-info')) document.getElementById('kembalian-info').classList.add('hidden');
        if(document.getElementById('cart-tax-zone')) document.getElementById('cart-tax-zone').classList.add('hidden');
        if(document.getElementById('cart-service-zone')) document.getElementById('cart-service-zone').classList.add('hidden');
        return;
    }
...
function renderPiutangList() {
    const listContainer = document.getElementById('piutang-list'); 
    ...
    listContainer.innerHTML = memberBerhutang.map(m => `
        <div class="bg-dark-8 p-5 rounded-xl border border-dark-4 flex flex-col gap-3 relative overflow-hidden shadow-sm">
            ...
            <div>
                <p class="text-sm font-black text-gray-100">${escapeHTML(m.nama)}</p>
                <p class="text-[10px] text-dark-2">HP: ${escapeHTML(m.id)}</p>
            </div>
            <div class="bg-dark-7 p-3 rounded-lg border border-dark-4">
                <p class="text-[10px] font-bold text-dark-2 uppercase tracking-wider">Total Hutang</p>
                <p class="text-lg font-black text-red-400">${toRupiah(m.hutang)}</p>
            </div>
...
function renderChart(labels, values) {
    ...
    chartInstance = new Chart(ctx, { 
        ...
        data: { 
            labels: labels, 
            datasets: [{ label: 'Qty Terjual', data: values, backgroundColor: '#1971c2', borderRadius: 6 }] 
        }, 
        options: { 
            responsive: true, maintainAspectRatio: false, 
            plugins: { legend: { display: false } }, 
            scales: { 
                x: { grid: { display: false }, ticks: { color: '#909296', font: { family: 'Inter', size: 10 } } }, 
                y: { grid: { color: '#373A40' }, ticks: { color: '#909296', font: { family: 'Inter', size: 10 }, precision: 0 } } 
            } 
        } 
    });
}
...
function renderShiftLogs() {
    const tbody = document.getElementById('shift-log-list'); 
    if(!tbody) return;
...
function updateHoldCountBadge() { 
    ...
    if(counts > 0) badge.classList.remove('hidden'); 
...
function renderHoldModalList() {
    ...
    if (heldBills.length === 0) { 
        listContainer.innerHTML = `<p class="text-xs text-dark-2 italic text-center py-4">Kosong.</p>`; 
        return; 
    }
        ...
        <div class="bg-dark-8 p-4 rounded-xl border border-dark-4 flex justify-between items-center gap-3">
            <div class="flex-1 min-w-0">
                <div class="flex justify-between items-center mb-1">
                    <span class="font-bold text-xs text-amber-400 truncate">${escapeHTML(bill.tag)}</span>
                </div>
...
function renderPemasokList() {
    const container = document.getElementById('pemasok-list'); 
    if(!container) return;
    
    if(databasePemasok.length === 0) { 
        container.innerHTML = `<p class="col-span-full text-xs text-dark-2 italic text-center py-8">Belum ada data Pemasok. Silakan tambahkan.</p>`; 
        return; 
    }
...
function renderAdminVouchers() {
    ...
    if(!tbody) return; 
    const vouchers = globalSettings.vouchers || {}; 
    const keys = Object.keys(vouchers);
    
    if(keys.length === 0) { 
        tbody.innerHTML = `<tr><td colspan="4" class="text-center py-4 text-dark-2 italic">Belum ada voucher aktif.</td></tr>`; 
        return; 
    }
    
    tbody.innerHTML = keys.map(k => { 
        const v = vouchers[k]; 
        const label = v.type === 'percent' ? `${v.value}%` : toRupiah(v.value); 
        return `
    ...
    try {
        ...
        request.onsuccess = async () => {
            ...
            if (offlineLogs.length > 0) { 
                ...
                if(failedLogs.length > 0) localStorage.setItem('pos_offline_logs', JSON.stringify(failedLogs)); 
                else localStorage.removeItem('pos_offline_logs'); 
            }
            if(successCount > 0) { 
                await logActivity("SYNC_OFFLINE", `Sukses sinkron ${successCount} transaksi.`); 
                alert(`Koneksi Stabil! ${successCount} data offline berhasil diunggah.`); 
            }
...
async function logActivity(actionType, actionDetails) {
    ...
    if (!navigator.onLine) { 
        ...
        const offlineLogs = JSON.parse(localStorage.getItem('pos_offline_logs') || '[]'); 
        offlineLogs.push(logObj); localStorage.setItem('pos_offline_logs', JSON.stringify(offlineLogs)); 
        return; 
    }
...
document.addEventListener("keydown", async (e) => {
    ...
    if (e.key === 'Enter' && barcodeBuffer.length > 0) {
        ...
        if (b) { 
            ...
            if (searchInput) { searchInput.value = ""; kataKunciPencarian = ""; kasirItemLimit = 36; renderKatalogKasir(); } 
        ...
        if (navigator.onLine) {
            try { 
                ...
                if (userDocSnap.exists()) { 
                    currentUserRole = userDocSnap.data().role || "kasir"; localStorage.setItem("pos_user_role", currentUserRole); 
                } else { 
                    currentUserRole = "kasir"; await setDoc(doc(db, "pengguna", user.uid), { email: user.email, role: "kasir", nama: user.email.split('@')[0] }); 
                } 
...
document.getElementById('login-form')?.addEventListener('submit', async (e) => {
    e.preventDefault(); 
    if (!navigator.onLine) return alert("Peringatan: Koneksi internet dibutuhkan untuk masuk!");
    try { 
...
document.getElementById('btn-logout')?.addEventListener('click', async () => { 
    ...
    if(confirm("Keluar dari sistem?")) { 
        matikanSemuaListener(); 
        try { await signOut(auth); } catch (e) {} 
        finally { sessionStorage.removeItem('pos_admin_authorized'); localStorage.clear(); location.reload(); } 
    } 
});
...
function checkActiveShift(uid) {
    ...
    unsubscribeActiveShift = onSnapshot(query(shiftsRef, where("userId", "==", uid), where("status", "==", "buka"), limit(1)), (snapshot) => {
        if (!snapshot.empty) { 
            ...
            localStorage.setItem("pos_cached_shift", JSON.stringify(activeShiftSession)); updateShiftUI(true); 
        } else if(navigator.onLine) { 
            activeShiftSession = null; localStorage.removeItem("pos_cached_shift"); updateShiftUI(false); 
        }
    });
}
...
window.triggerBukaShift = () => {
    ...
    document.getElementById('shift-input-label').textContent = "Uang Modal Fisik di Laci (Rp)"; 
    ...
    form.onsubmit = async (e) => {
        ...
        finally { if(btnSubmit) { btnSubmit.disabled = false; btnSubmit.textContent = "Buka Sesi"; } form.reset(); }
    };
...
window.triggerTutupShift = () => {
    ...
    document.getElementById('shift-input-label').textContent = "Uang Fisik Aktual di Laci (Rp)"; 
    ...
    form.onsubmit = async (e) => {
        ...
        finally { if(btnSubmit) { btnSubmit.disabled = false; btnSubmit.textContent = "Tutup Shift"; } form.reset(); }
    };
    document.getElementById('shift-modal')?.classList.remove('hidden');
};

// ==========================================
// VOUCHER & METODE BAYAR
// ==========================================
window.tambahVoucherAdmin = async () => {
    ...
    const val = parseInputRibuan(document.getElementById('new-voucher-value').value);
    
    if(!code || isNaN(val) || val <= 0) return alert("Data voucher tidak valid!");
    let currentVouchers = globalSettings.vouchers || {}; 
    currentVouchers[code] = { type: type, value: val };
    try { 
        await setDoc(doc(db, "pengaturan", "global"), { vouchers: currentVouchers }, { merge: true }); 
        document.getElementById('new-voucher-code').value = ""; 
        document.getElementById('new-voucher-value').value = ""; 
...
window.hapusVoucherAdmin = async (code) => {
    ...
    try { await setDoc(doc(db, "pengaturan", "global"), { vouchers: currentVouchers }, { merge: true }); } 
...
window.setQuickCash = (amount) => {
    ...
    if (amount === 'pas') { cashInput.value = formatInputRibuan(globalGrandTotal); } 
    else if (amount === 'clear') { cashInput.value = ""; } 
    else { cashInput.value = formatInputRibuan(amount); }
    hitungUangKembalian();
};
...
document.getElementById('btn-checkout')?.addEventListener('click', async (e) => {
    ...
    const trxData = { 
        ...
        items: [...keranjang], subtotal: globalSubtotal, diskon: globalDiskon, pajak: globalTaxAmount, serviceCharge: globalServiceAmount, totalAkhir: globalGrandTotal, totalModal: totalModalHPP, profit: totalProfit,
        namaKasir: (auth.currentUser ? auth.currentUser.email.split('@')[0] : 'Sistem'), 
        memberId: activeMember ? activeMember.id : null, memberName: activeMember ? activeMember.nama : null, voucherDigunakan: appliedVoucher ? document.getElementById('voucher-code')?.value.toUpperCase() : null, shiftId: activeShiftSession.id
    };
    ...
    if (isSplitPayment) {
        ...
        trxData.kembalian = splitDetails.kembalian; trxData.splitDetails = splitDetails;
        ...
        trxData.metodePembayaran = "Tunai"; 
        trxData.uangBayar = Math.max(0, parseInputRibuan(document.getElementById('cash-paid').value)); 
        trxData.kembalian = trxData.uangBayar - globalGrandTotal; 
    ...
    try {
        ...
        if (!isOnlineSuccess) {
            ...
            activeShiftSession.totalPenjualan += globalGrandTotal; activeShiftSession.totalTunai += tunaiMasukLaci; 
            localStorage.setItem("pos_cached_shift", JSON.stringify(activeShiftSession)); 
        ...
        keranjang = []; localStorage.removeItem("pos_recovery_cart"); appliedVoucher = null; 
        if(document.getElementById('voucher-code')) document.getElementById('voucher-code').value = ""; 
        if(document.getElementById('cash-paid')) document.getElementById('cash-paid').value = "";
        
        document.getElementById('pay-method-cash')?.click(); 
        document.getElementById('btn-remove-member')?.click(); 
        renderKeranjang(); applyFiltersAndStats(); window.switchCartTab('list');
        
    } catch(e) { 
        alert("GAGAL MEMPROSES TRANSAKSI: " + e.message); 
    } finally { 
        btnCheckout.disabled = false; btnCheckout.textContent = "Selesaikan Bayar"; 
    }
});
...
function cetakStrukThermal(data) {
    ...
    printArea.innerHTML = `<div style="font-family:monospace; color:black; max-width:300px; margin:0 auto; padding:10px;">
        ...
        <div style="font-size:10px;">
            ...
            ${(data.serviceCharge || 0) > 0 ? `<div style="display:flex; justify-content:space-between; margin-bottom:2px;"><span>Service:</span><span>+${toRupiah(data.serviceCharge)}</span></div>` : ''}
        ...
        <div style="border-top:1px dashed black; margin:8px 0;"></div><div style="text-align:center; font-size:10px; margin-top:10px;"><p style="margin:0; font-weight:bold;">${escapeHTML(globalSettings.footerStruk)}</p></div>
    </div>`;
...
itemForm?.addEventListener('submit', async (e) => {
    ...
    if (barcodeInput !== "") { 
        ...
        if (isDuplicate) return alert(`Barcode sudah dipakai: ${isDuplicate.nama}`); 
    }
    const btnSubmit = document.getElementById('btn-submit'); let origText = ""; 
...
window.duplikatBarang = (id) => {
    ...
    document.getElementById('item-id').value = ""; document.getElementById('item-barcode').value = ""; 
    document.getElementById('item-name').value = item.nama + " (Copy)"; document.getElementById('item-category').value = item.kategori || "Umum"; 
    document.getElementById('item-notes').value = item.catatan || ""; document.getElementById('item-cost').value = formatInputRibuan(item.cost); 
    document.getElementById('item-price').value = formatInputRibuan(item.harga); document.getElementById('item-stock').value = formatInputRibuan(item.stok);
...
window.editBarang = (id) => {
    ...
    document.getElementById('item-name').value = item.nama; document.getElementById('item-category').value = item.kategori || "Umum"; 
    document.getElementById('item-notes').value = item.catatan || ""; document.getElementById('item-cost').value = formatInputRibuan(item.cost); 
    document.getElementById('item-price').value = formatInputRibuan(item.harga); document.getElementById('item-stock').value = formatInputRibuan(item.stok);
    const supSelect = document.getElementById('item-supplier'); if (supSelect) supSelect.value = item.supplierId || "";
    document.getElementById('btn-cancel')?.classList.remove('hidden'); window.scrollTo({ top: 0, behavior: 'smooth' });
...
window.hapusBarang = async (id) => { 
    ...
    const item = databaseBarang.find(x => x.id === id); if(!item) return; 
    if(confirm(`Hapus permanen ${item.nama}?`)) { await deleteDoc(doc(db, "barang", id)); } 
};
...
window.tambahKeKeranjang = (id) => {
    ...
    if (existing) { 
        if(existing.qty >= item.stok) return alert("Melebihi stok gudang maksimal!"); 
        existing.qty++; 
    } else { 
        keranjang.push({ id: item.id, nama: item.nama, harga: item.harga||0, cost: item.cost||0, qty: 1 }); 
    } 
...
window.ubahQtyCart = (id, delta) => {
    ...
    if (keranjang[index].qty + delta <= 0) {
        if (sessionStorage.getItem("pos_admin_authorized") === "true") { 
            keranjang.splice(index, 1); localStorage.setItem("pos_recovery_cart", JSON.stringify(keranjang)); renderKeranjang(); 
        } else { 
            window.itemAkanDihapus = id; document.getElementById('pin-modal').classList.remove('hidden'); 
...
// ==========================================
// LISTENER METODE BAYAR & HOLD BILL
// ==========================================
document.getElementById('pay-method-cash')?.addEventListener('click', () => { 
    resetPaymentUI(); selectedPaymentMethod = 'Tunai'; 
    const btn = document.getElementById('pay-method-cash'); if(btn) btn.className = "py-2.5 text-[11px] font-bold bg-mantine-blue text-white rounded-md transition-all shadow"; 
    document.getElementById('cash-paid')?.classList.remove('hidden'); document.getElementById('quick-cash-zone')?.classList.remove('hidden'); hitungUangKembalian(); 
});
document.getElementById('pay-method-noncash')?.addEventListener('click', () => { 
    resetPaymentUI(); selectedPaymentMethod = 'Non-Tunai'; 
    const btn = document.getElementById('pay-method-noncash'); if(btn) btn.className = "py-2.5 text-[11px] font-bold bg-mantine-blue text-white rounded-md transition-all shadow"; 
    document.getElementById('noncash-ref')?.classList.remove('hidden'); hitungUangKembalian(); 
});
document.getElementById('pay-method-kasbon')?.addEventListener('click', () => { 
    if(!activeMember) { alert("Pilih Pelanggan terlebih dahulu untuk melakukan Kasbon!"); return; } 
    resetPaymentUI(); selectedPaymentMethod = 'Kasbon'; 
    const btn = document.getElementById('pay-method-kasbon'); if(btn) btn.className = "py-2.5 text-[11px] font-bold bg-amber-500 text-white rounded-md transition-all shadow"; 
    hitungUangKembalian(); 
});

document.getElementById('pay-method-split')?.addEventListener('click', () => {
    if(keranjang.length === 0) return alert("Keranjang kosong!");
    resetPaymentUI(); isSplitPayment = true; selectedPaymentMethod = 'Split'; 
    const btn = document.getElementById('pay-method-split'); if(btn) btn.className = "w-full py-2.5 mb-3 text-[11px] font-bold text-white bg-mantine-blue rounded-lg shadow-lg shadow-mantine-blue/30 transition-all";
    document.getElementById('split-total-tagihan').textContent = toRupiah(globalGrandTotal); 
    document.getElementById('split-amount-1').value = ""; document.getElementById('split-amount-2').value = formatInputRibuan(globalGrandTotal); 
    document.getElementById('split-modal').classList.remove('hidden');
});

window.batalSplitPayment = () => { document.getElementById('split-modal').classList.add('hidden'); document.getElementById('pay-method-cash')?.click(); };
...
document.getElementById('btn-save-split')?.addEventListener('click', () => {
    ...
    document.getElementById('btn-checkout').textContent = kembalianSplit > 0 ? `BAYAR (KEMBALI ${toRupiah(kembalianSplit)})` : `BAYAR (SPLIT)`;
});

document.getElementById('btn-hold-bill')?.addEventListener('click', () => {
    if (keranjang.length === 0) return alert("Keranjang kosong!");
    let holdName = prompt("Nama Penanda (cth: Meja 5):"); if (holdName === null) return; holdName = holdName.trim() || `Order #${Date.now().toString().slice(-4)}`;
    const heldBills = JSON.parse(localStorage.getItem('pos_held_bills') || '[]');
    heldBills.push({ id: Date.now().toString(), tag: holdName, waktu: new Date().toLocaleString('id-ID'), items: keranjang, diskon: 0, activeMember: activeMember, voucher: appliedVoucher, voucherCode: document.getElementById('voucher-code')?.value });
    localStorage.setItem('pos_held_bills', JSON.stringify(heldBills));
    keranjang = []; localStorage.removeItem("pos_recovery_cart"); activeMember = null; localStorage.removeItem("pos_recovery_member"); appliedVoucher = null; 
    if(document.getElementById('voucher-code')) document.getElementById('voucher-code').value = ""; 
    document.getElementById('btn-remove-member')?.click(); renderKeranjang(); updateHoldCountBadge(); alert("Pesanan ditangguhkan."); window.switchCartTab('list');
});
...
window.loadHeldBill = async (id) => {
    if (keranjang.length > 0 && !confirm("Ganti keranjang saat ini dengan pesanan tertunda?")) return;
    ...
    if (idx > -1) {
        ...
        if (bill.activeMember && bill.activeMember.id) { 
            if(navigator.onLine) { 
                try { const snap = await getDoc(doc(db, "members", bill.activeMember.id)); if(snap.exists()) activeMember = { id: snap.id, ...snap.data() }; } catch(e) { activeMember = bill.activeMember; } 
            } else { activeMember = bill.activeMember; } 
            localStorage.setItem("pos_recovery_member", JSON.stringify(activeMember)); showActiveMemberUI(); 
        ...
        document.getElementById('hold-modal')?.classList.add('hidden'); renderKeranjang(); updateHoldCountBadge(); window.switchCartTab('list');
    }
};
...
document.getElementById('btn-check-member')?.addEventListener('click', async () => {
    ...
    try {
        ...
        if (docSnap.exists()) { 
            activeMember = { id: phone, ...docSnap.data() }; localStorage.setItem("pos_recovery_member", JSON.stringify(activeMember)); showActiveMemberUI(); 
        } else { 
            if (confirm(`Member ${phone} belum terdaftar. Daftarkan?`)) { document.getElementById('member-reg-phone').value = phone; document.getElementById('member-reg-name').value = ""; document.getElementById('member-modal').classList.remove('hidden'); } 
        }
...
document.getElementById('btn-remove-member')?.addEventListener('click', () => { 
    ...
    renderKeranjang(); document.getElementById('pay-method-cash')?.click(); 
});
...
window.toggleSortGudang = () => { 
    ...
    gudangItemLimit = 30; renderGudangList(); 
};

window.startVoiceSearchGudang = () => {
    const btn = document.getElementById('btn-voice-search'), SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition; 
    if (!SpeechRecognition) { alert("Maaf, browser Anda tidak mendukung Pencarian Suara."); return; }
    ...
    recognition.onresult = function(event) { 
        const speechResult = event.results[0][0].transcript; const searchInput = document.getElementById('gudang-search'); 
        if(searchInput) { searchInput.value = speechResult; kataKunciGudang = speechResult.toLowerCase(); gudangItemLimit = 30; renderGudangList(); } 
    };
    recognition.onerror = function(event) { alert("Gagal mendengarkan suara."); if(btn) { btn.classList.remove('bg-red-500', 'animate-pulse'); btn.textContent = "🎤"; } };
...
window.startVoiceSearchKasir = () => {
    const btn = document.getElementById('btn-voice-search-kasir'), SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition; 
    if (!SpeechRecognition) { alert("Maaf, browser Anda tidak mendukung Pencarian Suara."); return; }
    const recognition = new SpeechRecognition(); recognition.lang = 'id-ID'; recognition.interimResults = false; recognition.maxAlternatives = 1;
    recognition.onstart = function() { if(btn) { btn.classList.add('bg-red-500', 'animate-pulse'); btn.textContent = "🎙️"; } };
    recognition.onresult = function(event) { 
        const speechResult = event.results[0][0].transcript; const searchInput = document.getElementById('kasir-search'); 
        if(searchInput) { searchInput.value = speechResult; kataKunciPencarian = speechResult.toLowerCase(); kasirItemLimit = 36; renderKatalogKasir(); } 
    };
    recognition.onerror = function(event) { alert("Gagal mendengarkan suara."); if(btn) { btn.classList.remove('bg-red-500', 'animate-pulse'); btn.textContent = "🎤"; } };
    recognition.onend = function() { if(btn) { btn.classList.remove('bg-red-500', 'animate-pulse'); btn.textContent = "🎤"; } }; recognition.start();
};
...
document.getElementById('btn-apply-voucher')?.addEventListener('click', () => {
    ...
    if (activeVouchers[code]) { 
        appliedVoucher = activeVouchers[code]; alert(`✅ Voucher ${code} berhasil diklaim!`); hitungUangKembalian(); 
    } else { 
        alert("❌ Kode Voucher tidak valid atau kadaluarsa."); appliedVoucher = null; document.getElementById('voucher-code').value = ""; hitungUangKembalian(); 
    }
});
...
document.getElementById('btn-submit-piutang')?.addEventListener('click', async () => {
    ...
    if (!navigator.onLine) {
        ...
        document.getElementById('btn-submit-piutang').textContent = "Lunasi"; return;
    }
    ...
    try {
        ...
        await addDoc(salesRef, { waktu: serverTimestamp(), tipe: "pelunasan_piutang", totalAkhir: inputVal, profit: 0, metodePembayaran: "Pelunasan Hutang (Tunai)", namaKasir: auth.currentUser?.email.split('@')[0], memberId: piutangAktifDipilih.id, memberName: piutangAktifDipilih.nama }); 
...
// ==========================================
// EXPORT & IMPORT EXCEL GUDANG / RIWAYAT
// ==========================================
document.getElementById('btn-export-gudang')?.addEventListener('click', () => {
    ...
    if (typeof XLSX !== 'undefined') { 
        const worksheet = XLSX.utils.json_to_sheet(dataExcel); const workbook = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(workbook, worksheet, "Data_Gudang"); XLSX.writeFile(workbook, `Data_Gudang_${new Date().toISOString().split('T')[0]}.xlsx`); 
...
document.getElementById('btn-export-excel')?.addEventListener('click', () => {
    ...
    try {
        ...
        if (typeof XLSX !== 'undefined') { 
            const worksheet = XLSX.utils.json_to_sheet(dataExcel); const maxCols = Object.keys(dataExcel[0] || {}).length; worksheet['!cols'] = Array(maxCols).fill({ wch: 20 }); const workbook = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(workbook, worksheet, "Laporan Penjualan"); XLSX.writeFile(workbook, `Laporan_Penjualan_${fileNameDate}.xlsx`); 
        } else { alert("Pustaka Excel belum termuat."); }
    } catch (error) { alert("Terjadi kesalahan saat membuat file Excel."); }
});

// ==========================================
// BACKUP LOKAL & SIMPAN PENGATURAN
// ==========================================
